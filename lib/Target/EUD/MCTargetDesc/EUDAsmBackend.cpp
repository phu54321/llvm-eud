//===-- EUDAsmBackend.cpp - EUD Assembler Backend -------------------------===//
//
...
class EUDAsmBackend : public MCAsmBackend {
public:
  bool IsLittleEndian;

  EUDAsmBackend(bool IsLittleEndian)
    : MCAsmBackend(), IsLittleEndian(IsLittleEndian) {}
  ~EUDAsmBackend() override = default;
...
bool EUDAsmBackend::writeNopData(uint64_t Count, MCObjectWriter *OW) const {
  if ((Count % 8) != 0)
    return false;
...
void EUDAsmBackend::applyFixup(const MCAssembler &Asm, const MCFixup &Fixup,
...
std::unique_ptr<MCObjectWriter>
EUDAsmBackend::createObjectWriter(raw_pwrite_stream &OS) const {
  return createEUDELFObjectWriter(OS, 0, IsLittleEndian);
}

MCAsmBackend *llvm::createEUDAsmBackend(const Target &T,

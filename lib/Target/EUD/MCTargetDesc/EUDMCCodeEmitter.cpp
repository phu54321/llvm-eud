//===-- EUDMCCodeEmitter.cpp - Convert EUD code to machine code -----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the EUDMCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
class EUDMCCodeEmitter : public MCCodeEmitter {
  const MCInstrInfo &MCII;
  const MCRegisterInfo &MRI;
  bool IsLittleEndian;

public:
  EUDMCCodeEmitter(const MCInstrInfo &mcii, const MCRegisterInfo &mri,
                   bool IsLittleEndian)
      : MCII(mcii), MRI(mri), IsLittleEndian(IsLittleEndian) {}
  EUDMCCodeEmitter(const EUDMCCodeEmitter &) = delete;
  void operator=(const EUDMCCodeEmitter &) = delete;
  ~EUDMCCodeEmitter() override = default;
...
MCCodeEmitter *llvm::createEUDMCCodeEmitter(const MCInstrInfo &MCII,
                                            const MCRegisterInfo &MRI,
                                            MCContext &Ctx) {
...
unsigned EUDMCCodeEmitter::getMachineOpValue(const MCInst &MI,
  ...
  if (MI.getOpcode() == EUD::JAL)
    // func call name
    Fixups.push_back(MCFixup::create(0, Expr, FK_PCRel_4));
  else if (MI.getOpcode() == EUD::LD_imm64)
...
void EUDMCCodeEmitter::encodeInstruction(const MCInst &MI, raw_ostream &OS,
                                         SmallVectorImpl<MCFixup> &Fixups,
                                         const MCSubtargetInfo &STI) const {
  ...
  if (Opcode == EUD::LD_imm64 || Opcode == EUD::LD_pseudo) {
...
// Encode EUD Memory Operand
uint64_t EUDMCCodeEmitter::getMemoryOpValue(const MCInst &MI, unsigned Op,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  uint64_t Encoding;
...
#include "EUDGenMCCodeEmitter.inc"

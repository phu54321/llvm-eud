//===-- EUDISelDAGToDAG.cpp - A dag to dag inst selector for EUD ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines a DAG pattern matching instruction selector for EUD,
// converting from a legalized dag to a EUD dag.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "eud-isel"

// Instruction Selector Implementation
namespace {

class EUDDAGToDAGISel : public SelectionDAGISel {
public:
  explicit EUDDAGToDAGISel(EUDTargetMachine &TM) : SelectionDAGISel(TM) {
    curr_func_ = nullptr;
  }

  StringRef getPassName() const override {
    return "EUD DAG->DAG Pattern Instruction Selection";
  }
...
// ComplexPattern used on EUD Load/Store instructions
bool EUDDAGToDAGISel::SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset) {
...
// ComplexPattern used on EUD FI instruction
bool EUDDAGToDAGISel::SelectFIAddr(SDValue Addr, SDValue &Base,
                                   SDValue &Offset) {
  SDLoc DL(Addr);
...
bool EUDDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void EUDDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    ...
    case Intrinsic::bpf_load_word: {
      ...
      SDValue R6Reg = CurDAG->getRegister(EUD::R6, MVT::i64);
...
void EUDDAGToDAGISel::PreprocessLoad(SDNode *Node,
...
void EUDDAGToDAGISel::PreprocessISelDAG() {
  ...
  //    EUD LOAD already has zero extension.
  //
  //    This also solved a correctness issue.
  //    In EUD socket-related program, e.g., __sk_buff->{data, data_end}
...
bool EUDDAGToDAGISel::getConstantFieldValue(const GlobalAddressSDNode *Node,
...
bool EUDDAGToDAGISel::fillGenericConstant(const DataLayout &DL,
                                          const Constant *CV,
                                          val_vec_type &Vals, uint64_t Offset) {
...
bool EUDDAGToDAGISel::fillConstantDataArray(const DataLayout &DL,
...
bool EUDDAGToDAGISel::fillConstantArray(const DataLayout &DL,
...
bool EUDDAGToDAGISel::fillConstantStruct(const DataLayout &DL,
...
void EUDDAGToDAGISel::PreprocessCopyToReg(SDNode *Node) {
...
void EUDDAGToDAGISel::PreprocessTrunc(SDNode *Node,
                                      SelectionDAG::allnodes_iterator &I) {
  ...
  // when the Reg operand comes from eud_load_[byte | half | word] for
  ...
  default:
    return;
  case 0xFFFFFFFF:
    match_load_op = EUD::LDW;
    break;
  case 0xFFFF:
    match_load_op = EUD::LDH;
    break;
  case 0xFF:
    match_load_op = EUD::LDB;
    break;
  }
...
bool EUDDAGToDAGISel::checkLoadDef(unsigned DefReg, unsigned match_load_op) {
...
FunctionPass *llvm::createEUDISelDag(EUDTargetMachine &TM) {
  return new EUDDAGToDAGISel(TM);
}
